...
- **Add a theme**: add one entry to `M.themes` + `M.order` in
  ...
  `background = 'dark' | 'light'`; `separator_fg` is optional — only set
  it to override the centralized default for this theme)
...
- **Separator / float-border brightness**: centralized via
  `M.SEPARATOR_DARK_FG` and `M.SEPARATOR_LIGHT_FG` in `util/themes.lua`.
  A `ColorScheme` autocmd in `core/autocmds.lua` applies the resolved fg
  to `WinSeparator` and `FloatBorder`. Resolution precedence:
  per-theme `separator_fg` (override) → background-based global → `white`
  fallback. Tweak the two module globals to re-tune globally.
