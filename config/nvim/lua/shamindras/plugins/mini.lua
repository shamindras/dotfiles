return {
  ...
  config = function()
    ...
    vim.keymap.set({ 'n', 'x' }, '<leader>xo', function()
      -- Open only the first URL. _get_urls() lists the same link from
      -- multiple providers (LSP documentLink + treesitter), whose strings
      -- can differ (percent-encoding), so stock `gx`'s loop opens two tabs.
